...
- Inspected `README.md`, `package.json`, the active OpenSpec design/tasks and reviewer-workspace spec: the financial application is not implemented; `apps/web/` is a proposed React/TypeScript/Vite path. No existing application components, shadcn configuration or Storybook catalogue were found. This handoff specifies future UI work, not a working application or completed acceptance tests.
