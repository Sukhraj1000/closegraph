...
def build_services(*,seed=False,migrate=False,pdf_provider=None):
    ...
    accounts=[DevAccount.create(name,os.environ['CLOSEGRAPH_'+name.upper()+'_PASSWORD'],role,scopes) for name,role in [('accountant','PREPARER'),('account_manager','REVIEWER')]]
    ...
    if migrate:
        from closegraph.collections.account_migration import migrate_business_accounts
        migrate_business_accounts(services.collections)
    if seed:
        actor=Actor(actor_id='accountant',role='PREPARER')
        for scope in scopes:
