...
export function findingTitle(finding: Finding) {
 const key = ['reference','unique'].includes(finding.kind) && Array.isArray(finding.observed)
  ...
  : '';
 const affected = ['reference','unique','required','totals'].includes(finding.kind) && typeof finding.affected_count==='number' && finding.affected_count>0
...
export function fileFailure(source:SourceFile, issues:Issue[] = []) {
 const failures=[...(source.issues??[]),...issues.filter(issue=>!issue.source_id||issue.source_id===source.id)].filter(issue=>!issue.resolved);
