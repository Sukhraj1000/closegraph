...
function mock(collection=base){vi.spyOn(CollectionsApi.prototype,'history').mockResolvedValue([]);vi.spyOn(CollectionsApi.prototype,'participants').mockResolvedValue([]);vi.spyOn(CollectionsApi.prototype,'list').mockResolvedValue([collection]);vi.spyOn(CollectionsApi.prototype,'get').mockResolvedValue(collection);vi.spyOn(CollectionsApi.prototype,'rows').mockResolvedValue(page);}
...
describe('collection evidence review',()=>{
 it('shows source coordinates and sends a reasoned correction for the exact revision',async()=>{
  ...
  await user.click(await screen.findByRole('button',{name:'Documents'}));await user.click(await screen.findByRole('button',{name:'Amount, row 1: 12.20'}));
 ...
 it('records unresolved concerns rather than relabelling extraction as accepted',async()=>{
  mock();const edit=vi.spyOn(CollectionsApi.prototype,'edit').mockResolvedValue({...base,version:4}),accept=vi.spyOn(CollectionsApi.prototype,'accept');const user=userEvent.setup();mount();await screen.findByRole('heading',{name:'Fictional source review'});await user.click(screen.getByRole('button',{name:'Documents'}));
 ...
 it('refreshes a conflicting revision and never silently retries a correction',async()=>{
  mock();const edit=vi.spyOn(CollectionsApi.prototype,'edit').mockRejectedValue(new ApiError(409,'Stale revision'));const user=userEvent.setup();mount();await user.click(await screen.findByRole('button',{name:'Documents'}));await user.click(await screen.findByRole('button',{name:'Amount, row 1: 12.20'}));await user.type(screen.getByRole('textbox',{name:'Reason and source evidence'}),'Inspect original row');await user.click(screen.getByRole('button',{name:'Save extraction decision'}));
 ...
 it('requires an independent reviewer and inspection before approval, and keeps exports gated',async()=>{
  ...
  mock(ready);const approve=vi.spyOn(CollectionsApi.prototype,'review').mockResolvedValue({...ready,status:'APPROVED',version:4});const user=userEvent.setup();mount({...session,username:'fictional-reviewer',role:'REVIEWER'});await user.click(await screen.findByRole('button',{name:'Outputs'}));
 ...
 it('does not expose approval controls to the preparer',async()=>{
  mock({...base,status:'READY_FOR_REVIEW',datasets:[{...base.datasets[0],kind:'output'}]});const user=userEvent.setup();mount();await user.click(await screen.findByRole('button',{name:'Outputs'}));expect(screen.queryByRole('button',{name:'Approve exact output'})).not.toBeInTheDocument();expect(screen.getByRole('button',{name:'Export XLSX'})).toBeDisabled();
 });
