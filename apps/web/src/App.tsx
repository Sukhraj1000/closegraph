...
function Login({onLogin}:{onLogin:(session:Session)=>void}) {
 ...
 return <main className="login"><Card><div className="brand"><span className="brand-mark" aria-hidden/>CloseGraph</div><h1>Sign in to CloseGraph</h1><p className="muted">Review your fund reporting pack, trace questions to source evidence and see what changed.</p>{error&&<div role="alert"><Alert title="Sign-in failed" tone="danger">{error}</Alert></div>}<form onSubmit={submit} noValidate><fieldset className="login-accounts"><legend>Choose your account</legend>{roleAccounts.map(account=><button key={account.username} type="button" aria-pressed={username===account.username} disabled={pending} onClick={()=>{setUsername(account.username);setError('');}}>{account.label}</button>)}</fieldset><Field id={id+'-username'} label="Username"><Input id={id+'-username'} ref={ref} autoComplete="username" value={username} onChange={e=>setUsername(e.target.value)} required disabled={pending}/></Field><Field id={id+'-password'} label="Password"><Input id={id+'-password'} type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} required disabled={pending}/></Field><Button type="submit" disabled={pending}>{pending?'Signing in…':'Sign in'}</Button></form></Card></main>;
}
