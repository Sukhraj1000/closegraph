...
def validate_commit_identity(value):
    """Validate trusted metadata without normalising retry-sensitive strings."""
    if not isinstance(value, dict) or set(value) != {"name", "email"}:
        raise ValueError("Trusted commit_identity requires exactly name and email")
    for field in ("name", "email"):
        ...
        if (not isinstance(text, str) or not text.strip() or text != text.strip()
            ...
            raise ValueError("Invalid trusted commit_identity " + field)
    if not re.fullmatch(r"[^@\s]+@[^@\s]+", value["email"]):
        raise ValueError("Invalid trusted commit_identity email")
    return dict(value)
...
class GitHub:
    ...
    def publish(self, job, changes):
        ...
        identity = {**validate_commit_identity(job.get("commit_identity")), "date": job["commit_date"]}
        base = job["base"]
        entries = []
