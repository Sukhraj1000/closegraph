...
Open [CloseGraph](http://127.0.0.1:24173). Sign in using your own account: `accountant`, `account_manager`, `fund_manager` or `investor`, each with a separate password. There is no role switcher. Existing passwords are retained when updating an installed runtime. `init` creates distinct random local account passwords in the owner-only `.local/dev-runtime/.env`; it prints the location without printing secrets. The standard navigation is New review, Recent work and Requests. Existing Collections and native reporting services remain compatible with stored work. Use the explicitly labelled files in [fixtures/synthetic](fixtures/synthetic/README.md). These fictional rules are test authority, not general accounting treatment.
